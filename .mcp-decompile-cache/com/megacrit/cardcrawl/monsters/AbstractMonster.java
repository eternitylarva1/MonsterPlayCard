 ...
 *  com.megacrit.cardcrawl.monsters.AbstractMonster$EnemyType
 *  com.megacrit.cardcrawl.monsters.AbstractMonster$Intent
 *  com.megacrit.cardcrawl.monsters.EnemyMoveInfo
 ...
 *  com.megacrit.cardcrawl.vfx.BobEffect
 ...
 *  com.megacrit.cardcrawl.vfx.combat.DeckPoofEffect
 *  com.megacrit.cardcrawl.vfx.combat.FlashIntentEffect
 *  com.megacrit.cardcrawl.vfx.combat.HbBlockBrokenEffect
 *  com.megacrit.cardcrawl.vfx.combat.HealEffect
...
extends AbstractCreature {
    private static final Logger logger = LogManager.getLogger((String)AbstractMonster.class.getName());
    ...
    public AbstractMonster(String name, String id, int maxHealth, float hb_x, float hb_y, float hb_w, float hb_h, String imgUrl, float offsetX, float offsetY, boolean ignoreBlights) {
        ...
        if (ModHelper.isModEnabled((String)"MonsterHunter")) {
        ...
        if (imgUrl != null) {
            this.img = ImageMaster.loadImage((String)imgUrl);
        }
    ...
    public void renderTip(SpriteBatch sb) {
        ...
        if (!this.tips.isEmpty()) {
            if (this.hb.cX + this.hb.width / 2.0f < TIP_X_THRESHOLD) {
                TipHelper.queuePowerTips((float)(this.hb.cX + this.hb.width / 2.0f + TIP_OFFSET_R_X), (float)(this.hb.cY + TipHelper.calculateAdditionalOffset((ArrayList)this.tips, (float)this.hb.cY)), (ArrayList)this.tips);
            } else {
                TipHelper.queuePowerTips((float)(this.hb.cX - this.hb.width / 2.0f + TIP_OFFSET_L_X), (float)(this.hb.cY + TipHelper.calculateAdditionalOffset((ArrayList)this.tips, (float)this.hb.cY)), (ArrayList)this.tips);
            }
        }
    }

    private void updateIntentTip() {
        switch (1.$SwitchMap$com$megacrit$cardcrawl$monsters$AbstractMonster$Intent[this.intent.ordinal()]) {
            case 1: {
    ...
    public void setMove(String moveName, byte nextMove, Intent intent) {
        if (intent == Intent.ATTACK || intent == Intent.ATTACK_BUFF || intent == Intent.ATTACK_DEFEND || intent == Intent.ATTACK_DEBUFF) {
            for (int i = 0; i < 8; ++i) {
                AbstractDungeon.effectsQueue.add(new TextAboveCreatureEffect(MathUtils.random((float)((float)Settings.WIDTH * 0.25f), (float)((float)Settings.WIDTH * 0.75f)), MathUtils.random((float)((float)Settings.HEIGHT * 0.25f), (float)((float)Settings.HEIGHT * 0.75f)), "ENEMY MOVE " + moveName + " IS SET INCORRECTLY! REPORT TO DEV", Color.RED.cpy()));
            }
    ...
    private void renderDamageRange(SpriteBatch sb) {
        if (this.intent.name().contains("ATTACK")) {
            if (this.isMultiDmg) {
                FontHelper.renderFontLeftTopAligned((SpriteBatch)sb, (BitmapFont)FontHelper.topPanelInfoFont, (String)(Integer.toString(this.intentDmg) + "x" + Integer.toString(this.intentMultiAmt)), (float)(this.intentHb.cX - 30.0f * Settings.scale), (float)(this.intentHb.cY + this.bobEffect.y - 12.0f * Settings.scale), (Color)this.intentColor);
            } else {
                FontHelper.renderFontLeftTopAligned((SpriteBatch)sb, (BitmapFont)FontHelper.topPanelInfoFont, (String)Integer.toString(this.intentDmg), (float)(this.intentHb.cX - 30.0f * Settings.scale), (float)(this.intentHb.cY + this.bobEffect.y - 12.0f * Settings.scale), (Color)this.intentColor);
            }
        }
    }
    ...
    private void renderName(SpriteBatch sb) {
        this.hoverTimer = !this.hb.hovered ? MathHelper.fadeLerpSnap((float)this.hoverTimer, (float)0.0f) : (this.hoverTimer += Gdx.graphics.getDeltaTime());
        if (!(AbstractDungeon.player.isDraggingCard && AbstractDungeon.player.hoveredCard != null && AbstractDungeon.player.hoveredCard.target != AbstractCard.CardTarget.ENEMY || this.isDying)) {
            this.nameColor.a = this.hoverTimer != 0.0f ? (this.hoverTimer * 2.0f > 1.0f ? 1.0f : this.hoverTimer * 2.0f) : MathHelper.slowColorLerpSnap((float)this.nameColor.a, (float)0.0f);
            ...
            sb.draw((TextureRegion)img, x - (float)img.packedWidth / 2.0f, y - (float)img.packedHeight / 2.0f, (float)img.packedWidth / 2.0f, (float)img.packedHeight / 2.0f, (float)img.packedWidth, (float)img.packedHeight, Settings.scale * tmp, Settings.scale * 2.0f, 0.0f);
            this.nameColor.a *= this.hbAlpha;
            FontHelper.renderFontCentered((SpriteBatch)sb, (BitmapFont)FontHelper.tipHeaderFont, (String)this.name, (float)x, (float)y, (Color)this.nameColor);
        }
    }
    ...
    public void useUniversalPreBattleAction() {
        if (ModHelper.isModEnabled((String)"Lethality")) {
        ...
        if (ModHelper.isModEnabled((String)"Time Dilation") && !this.id.equals("GiantHead")) {
    ...
    public void applyPowers() {
        ...
        if (applyBackAttack && !this.hasPower("BackAttack")) {
            AbstractDungeon.actionManager.addToTop((AbstractGameAction)new ApplyPowerAction((AbstractCreature)this, null, (AbstractPower)new BackAttackPower((AbstractCreature)this)));
        }
    ...
    public void removeSurroundedPower() {
        if (this.hasPower("BackAttack")) {
            AbstractDungeon.actionManager.addToTop((AbstractGameAction)new RemoveSpecificPowerAction((AbstractCreature)this, null, "BackAttack"));
        }
    }
    ...
    protected void onBossVictoryLogic() {
        ...
        if (AbstractDungeon.getCurrRoom().event == null) {
            ...
            if (GameActionManager.turn <= 1) {
                UnlockTracker.unlockAchievement((String)"YOU_ARE_NOTHING");
            }
            if (GameActionManager.damageReceivedThisCombat - GameActionManager.hpLossThisCombat <= 0) {
                UnlockTracker.unlockAchievement((String)"PERFECT");
    ...
    protected void onFinalBossVictoryLogic() {
        if (!(AbstractDungeon.ascensionLevel >= 20 && AbstractDungeon.bossList.size() == 2 || Settings.isEndless)) {
            ...
            if (CardCrawlGame.playtime <= 1200.0f) {
                UnlockTracker.unlockAchievement((String)"SPEED_CLIMBER");
            }
            if (AbstractDungeon.player.masterDeck.size() <= 5) {
                UnlockTracker.unlockAchievement((String)"MINIMALIST");
            }
            ...
            if (commonSenseUnlocked) {
                UnlockTracker.unlockAchievement((String)"COMMON_SENSE");
            }
            if (AbstractDungeon.player.relics.size() == 1) {
                UnlockTracker.unlockAchievement((String)"ONE_RELIC");
            }
    ...
    public static void playBossStinger() {
        ...
        if (AbstractDungeon.id.equals("TheEnding")) {
            ...
            switch (MathUtils.random((int)0, (int)3)) {
                case 0: {
